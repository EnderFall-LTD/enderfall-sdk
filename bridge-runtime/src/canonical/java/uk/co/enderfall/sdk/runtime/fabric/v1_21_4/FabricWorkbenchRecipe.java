...
/** Fabric 1.21.4 codec-backed portable positional recipe. */
final class FabricWorkbenchRecipe implements Recipe<FabricWorkbenchInput> {
    ...
    FabricWorkbenchRecipe(FabricRecipeBinding binding, List<Entry> ingredients, ItemStack result) {
    ...
    @Override public ItemStack assemble(FabricWorkbenchInput input, HolderLookup.Provider registries) {
        return result.copy();
    }
    ...
    static RecipeSerializer<FabricWorkbenchRecipe> serializer(java.util.function.Supplier<FabricRecipeBinding> binding) {
        return new RecipeSerializer<>() {
            @Override public MapCodec<FabricWorkbenchRecipe> codec() { return recipeCodec(binding.get()); }
            @Override @SuppressWarnings("deprecation")
            public StreamCodec<RegistryFriendlyByteBuf, FabricWorkbenchRecipe> streamCodec() {
                return recipeStreamCodec(binding.get());
            }
        };
    }

    private static MapCodec<FabricWorkbenchRecipe> recipeCodec(FabricRecipeBinding binding) {
        return RecordCodecBuilder.mapCodec(instance -> instance.group(
                Entry.CODEC.codec().listOf().fieldOf("ingredients").forGetter(recipe -> recipe.ingredients),
                ItemStack.CODEC.fieldOf("result").forGetter(recipe -> recipe.result)
        ).apply(instance, (ingredients, result) -> new FabricWorkbenchRecipe(binding, ingredients, result)));
    }

    private static StreamCodec<RegistryFriendlyByteBuf, FabricWorkbenchRecipe> recipeStreamCodec(
            FabricRecipeBinding binding) {
        return new StreamCodec<>() {
            @Override public FabricWorkbenchRecipe decode(RegistryFriendlyByteBuf buffer) {
                ...
                return new FabricWorkbenchRecipe(binding, ingredients, ItemStack.STREAM_CODEC.decode(buffer));
            }
