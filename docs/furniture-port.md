...
| 4. Recipes and richer screens | `common/menus/WorkstationMenu.java`, `client/screens/WorkstationScreen.java` | Secure text widgets and general server-owned selectable/filterable lists are implemented. Instant item workbenches now discover matching data-pack recipes and synchronize selectable result previews, required counts, pages and the selected result without consumer packets. Remaining work includes workstation search/category controls, richer recipe details and equivalent timed/fluid-machine presentation. |
