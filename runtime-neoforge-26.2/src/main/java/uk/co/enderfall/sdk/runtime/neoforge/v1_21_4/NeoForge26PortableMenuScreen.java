...
/** Minecraft 26.2 renderer for the portable synchronized menu contract. */
final class NeoForgePortableMenuScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.centeredText(font, view.state().resolve(view.spec().title()),
                left + view.spec().width() / 2, top + 12, 0xFFFFFFFF);
        for (MenuLabel label : view.spec().labels()) {
            String text = view.state().resolve(label.text());
            if (label.centered()) {
                graphics.centeredText(font, text, left + label.x(), top + label.y(), label.color());
            } else {
                graphics.text(font, text, left + label.x(), top + label.y(), label.color(), false);
            }
        }
    ...
    static void show(PortableMenuView view, Consumer<String> actionSender, Runnable closeSender) {
        ...
        client.execute(() -> client.gui.setScreen(new NeoForgePortableMenuScreen(view, actionSender, closeSender)));
    }

    static void update(long sessionId, MenuState state) {
        Minecraft client = Minecraft.getInstance();
        client.execute(() -> {
            if (client.gui.screen() instanceof NeoForgePortableMenuScreen screen && screen.sessionId() == sessionId) {
                screen.update(state);
            }
        });
    }
