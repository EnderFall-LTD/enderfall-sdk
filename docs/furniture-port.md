...
| 3. Items and durable data | `registry/ModComponents.java`, `common/items/WrenchItem.java` | Class-based portable item definitions, server-side right-click/use-on-block routing, declarative literal/translated tooltips, typed held-stack data, and expected-item-checked arbitrary carried-slot author/text updates backed by legacy NBT or component-era custom data are implemented. Add the reusable editable letter screen/session and declared wrench/hammer property policies. Verify copies, stacks, dropped items and reconnects preserve data. |
