...
| 3. Items and durable data | `registry/ModComponents.java`, `common/items/WrenchItem.java` | Class-based portable item definitions plus server-side right-click and use-on-block routing with position, hand and crouching context are implemented. Add portable letter author/text data persisted and synchronized, with legacy representation on pre-component targets, plus tooltips and declared wrench/hammer property policies. Verify copies, stacks, dropped items and reconnects preserve data. |
