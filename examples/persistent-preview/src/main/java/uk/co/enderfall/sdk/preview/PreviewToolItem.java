...
import uk.co.enderfall.sdk.api.item.InteractionPrediction;
...
public final class PreviewToolItem implements PortableItem {
    ...
    @Override public void onUse(ModContext context, InteractionEvent event) {
        // A crouched use is reserved for property selection. This guard also keeps
        // older clients/loaders from counting a predicted use-on-block fallback.
        if (event.sneaking()) {
            event.handle();
            return;
        }
    ...
    @Override public InteractionPrediction predictUseOnBlock(InteractionEvent event) {
        return event.target().equals(PreviewBlocks.ORIENTATION_TEST.id())
        ...
        event.blockLocation().ifPresent(location -> {
            ...
            var result = event.sneaking()
                    ...
                    : context.blockStates().cycleToolProperty(
                            block, location, CONFIGURATION_TOOL, selectedProperty,
                            proposed -> true,
                            committed -> finishCommittedChange(context, event, committed));
    ...
    private static void finishCommittedChange(ModContext context, InteractionEvent event,
            uk.co.enderfall.sdk.api.block.BlockToolResult change) {
