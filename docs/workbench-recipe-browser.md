...
counts, selected-result highlighting, paging buttons and mouse-wheel paging. Built-in
category arrows switch between **All**, **Blocks**, and **Items**, while the counter
shows the selected result and the total in that category. Clicking a result selects it;
taking the real output performs the craft.

The logical server discovers and sorts matching recipes by resource ID, validates every
selection and category, recalculates them whenever inputs change and consumes the
selected recipe's exact counted ingredients. Category classification uses the native
result item type on the logical server. The client only renders synchronized menu state
and sends Minecraft's normal menu-button action. A forged index, a stale recipe or
changed inputs cannot craft a different result.
...
layout and synchronization contract. Text search and consumer-defined recipe-category
tabs are separate future additions.

## Test it in the persistent preview

Place the Persistent Workbench and insert, from left to right, two iron ingots, one
redstone dust and one quartz. Seven matching output choices should appear across two
pages. The built-in categories should show four blocks and three items. Select choices,
change pages with the arrows or wheel, switch categories, then take the real output.
