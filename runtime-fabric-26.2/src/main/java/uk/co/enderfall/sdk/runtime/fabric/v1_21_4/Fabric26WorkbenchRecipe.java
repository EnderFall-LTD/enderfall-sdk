...
/** Minecraft 26.2 replacement for Fabric's portable positional recipe. */
final class FabricWorkbenchRecipe implements Recipe<FabricWorkbenchInput> {
    private final FabricRecipeBinding binding;
    private final List<Entry> ingredients;
    private final ItemStackTemplate result;

    FabricWorkbenchRecipe(FabricRecipeBinding binding, List<Entry> ingredients, ItemStackTemplate result) {
    ...
    @Override public boolean matches(FabricWorkbenchInput input, Level level) {
    ...
    @Override public ItemStack assemble(FabricWorkbenchInput input) { return result.create(); }
    @Override public boolean isSpecial() { return true; }
    @Override public boolean showNotification() { return true; }
    @Override public String group() { return binding.type().toString(); }
    @Override public RecipeSerializer<? extends Recipe<FabricWorkbenchInput>> getSerializer() {
        return binding.serializer();
    }
    @Override public RecipeType<? extends Recipe<FabricWorkbenchInput>> getType() { return binding.type(); }
    ...
    static RecipeSerializer<FabricWorkbenchRecipe> serializer(java.util.function.Supplier<FabricRecipeBinding> binding) {
        return new RecipeSerializer<>(recipeCodec(binding), recipeStreamCodec(binding));
    }
    private static MapCodec<FabricWorkbenchRecipe> recipeCodec(java.util.function.Supplier<FabricRecipeBinding> binding) {
        return RecordCodecBuilder.mapCodec(instance -> instance.group(
                Entry.CODEC.codec().listOf().fieldOf("ingredients").forGetter(recipe -> recipe.ingredients),
                ItemStackTemplate.CODEC.fieldOf("result").forGetter(recipe -> recipe.result)
        ).apply(instance, (ingredients, result) -> new FabricWorkbenchRecipe(binding.get(), ingredients, result)));
    }
    private static StreamCodec<RegistryFriendlyByteBuf, FabricWorkbenchRecipe> recipeStreamCodec(
            java.util.function.Supplier<FabricRecipeBinding> binding) {
        return new StreamCodec<>() {
            @Override public FabricWorkbenchRecipe decode(RegistryFriendlyByteBuf buffer) {
                FabricRecipeBinding resolved = binding.get();
                int size = buffer.readVarInt();
                if (size != resolved.inputSlots()) throw new IllegalArgumentException(
                ...
                return new FabricWorkbenchRecipe(resolved, ingredients, ItemStackTemplate.STREAM_CODEC.decode(buffer));
            }
            @Override public void encode(RegistryFriendlyByteBuf buffer, FabricWorkbenchRecipe recipe) {
