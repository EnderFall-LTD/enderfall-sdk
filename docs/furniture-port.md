...
| 3. Items and durable data | `registry/ModComponents.java`, `common/items/WrenchItem.java` | Class-based portable item definitions and server-side right-click routing are implemented. Add portable letter author/text data persisted and synchronized, with legacy representation on pre-component targets, plus use-on-block context, tooltips and wrench/hammer state changes. Verify copies, stacks, dropped items and reconnects preserve data. |
