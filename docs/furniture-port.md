...
| 4. Recipes and richer screens | `common/menus/WorkstationMenu.java`, `client/screens/WorkstationScreen.java` | Secure single/multiline text widgets now join state labels/buttons. Server-owned selectable/filterable paged lists, disabled-row enforcement, selection validation and synchronized updates are implemented. Recipe-registry discovery, item icons, mouse-wheel scrolling and synchronized rich results remain. Current positional recipes cover only part of this. |
