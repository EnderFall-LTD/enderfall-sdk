...
| 3. Items and durable data | `registry/ModComponents.java`, `common/items/WrenchItem.java` | Class-based portable item definitions, server-side right-click/use-on-block routing, declarative literal/translated tooltips, and typed held-stack data backed by legacy NBT or component-era custom data are implemented. Add secure arbitrary-slot author/text updates for the letter screen and declared wrench/hammer property policies. Verify copies, stacks, dropped items and reconnects preserve data. |
