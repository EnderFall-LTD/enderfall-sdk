    ...
    @Test void allTargetsUseOrdinaryConsumerBootstrapAndCompleteFeatureSources() throws Exception {
        for (String target : TargetCatalog.standard().targetIds()) {
            ...
            try (var files = Files.walk(output.resolve("java"))) {
                ...
                assertTrue(shapeSource.contains("onNeighborChanged(context)"), target);
                assertTrue(shapeSource.contains("onScheduledTick(context)"), target);
                assertTrue(shapeSource.contains("onPlaced(portableContext)"), target);
                boolean legacyNeighbor = target.startsWith("1.20.1-") || target.startsWith("1.21.1-");
                assertEquals(legacyNeighbor, shapeSource.contains("LevelAccessor level, BlockPos pos, BlockPos neighborPos"), target);
                assertEquals(!legacyNeighbor, shapeSource.contains("ScheduledTickAccess scheduledTicks"), target);
                assertTrue(shapeSource.contains("implements net.minecraft.world.level.block.SimpleWaterloggedBlock"), target);
                assertTrue(shapeSource.contains("specification.waterlogged()"), target);
                assertTrue(shapeSource.contains("BlockStateProperties.WATERLOGGED"), target);
                assertTrue(shapeSource.contains("Fluids.WATER.getSource(false)"), target);
                assertTrue(shapeSource.contains("scheduleAfterTicks().ifPresent(schedule)"), target);
                assertTrue(platformSource.contains("supportsScheduledBlockTicks() { return true; }"), target);
                assertTrue(platformSource.contains("supportsWaterloggedBlocks() { return true; }"), target);
