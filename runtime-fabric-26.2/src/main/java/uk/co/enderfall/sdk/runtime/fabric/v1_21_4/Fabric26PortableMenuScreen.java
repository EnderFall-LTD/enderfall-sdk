...
final class FabricPortableMenuScreen extends Screen {
    ...
    static void show(PortableMenuView view, Consumer<String> actionSender, Runnable closeSender) {
        ...
        client.execute(() -> client.gui.setScreen(new FabricPortableMenuScreen(view, actionSender, closeSender)));
    }

    static void update(long sessionId, MenuState state) {
        Minecraft client = Minecraft.getInstance();
        client.execute(() -> {
            if (client.gui.screen() instanceof FabricPortableMenuScreen screen && screen.sessionId() == sessionId) {
                screen.update(state);
            }
        });
    }
