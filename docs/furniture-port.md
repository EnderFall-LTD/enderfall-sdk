...
| 4. Recipes and richer screens | `common/menus/WorkstationMenu.java`, `client/screens/WorkstationScreen.java` | Secure single/multiline text widgets now join state labels/buttons. Server-owned selectable/filterable paged lists, disabled-row enforcement, selection validation, native item-stack icons/counts, hover details, selected-row marking, mouse-wheel paging and synchronized updates are implemented. Recipe-registry discovery and synchronized selected-recipe/result panels remain. Current positional recipes cover only part of this. |
